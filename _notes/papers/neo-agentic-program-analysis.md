...
description: 解析NEO如何用CodeQL搜索原语、跨服务数据流、LLM授权推理与Z3路径约束检测权限提升漏洞。
date: 2026-07-19
updated: 2026-07-19
...
status: reviewed
audience: 关注静态程序分析、微服务安全与LLM Agent的安全研究人员。
...
论文将LLM的语义推理能力与CodeQL的静态程序分析能力结合，用于检测多语言微服务中的权限提升漏洞。{% cite li2026neo %}
NEO在SAST之上提供面向Agent的窄接口。CodeQL提供代码结构和数据流证据，LLM评估操作的业务敏感性与授权充分性。
...
2. **特权操作依赖业务语义。** 数据库写入和文件访问容易列举，付款确认、优惠券分配、租户切换和角色修改等操作需要结合应用功能认定其敏感性。
...
工程中固化的内容通常是稳定的API契约，例如限定名称的方法第0个参数对应SQL查询，或某个注解标记HTTP入口。具体仓库的文件名和调用链会由提取器转换成关系事实，通用规则再对这些事实求解。因此，少量成熟模型可以覆盖大量使用相同语言和框架的代码库。
...
Joern查询通常从 `cpg` 开始，连续组合节点类型、遍历步骤、过滤器和属性，以获得目标调用及其实参，或获得指定方法的被调用函数。CodeQL以类型、谓词和关系约束声明结果集合，例如筛选名称为 `updateRole` 的 `MethodAccess` 并返回其位置。

Joern 的优势是图遍历直观，适合交互式探索；CodeQL 的优势是类型化查询、成熟语言库、跨过程数据流和安全查询生态。NEO 选择 CodeQL 作为后端，但论文也明确指出，代码搜索原语可以在 Joern 等其他分析后端上实现。

### 语法空间与API接口面的规模

这里的庞大指Agent必须在大量API和语义选项构成的组合空间中作出正确选择：
...
AST查找本质上是确定性关系查询。例如，调用名为 `updateRole` 的位置直接筛选已提取的调用节点。它不需要推测某次运行会发生什么，因此通常比数据流和调用图稳定。
...
其中包括赋值、实参与形参、返回值、库函数摘要以及查询作者补充的传播步骤。值流要求语义上的值保持，污点追踪还允许编码、拼接等非值保持但影响关系仍存在的步骤。全局数据流再对这些边做跨过程可达性求解。

调用图的直接调用较容易解析；接口、继承和虚方法调用必须计算可能目标。CodeQL中类似 `polyCalls` 的谓词会将所有静态上可能的重写实现纳入结果。
...
传统规则容易列出通用sink，却难以穷举应用特有的业务操作。NEO首先阅读README等自然语言材料理解系统功能，再由LLM生成名称或AST查询，例如以 `update.*` 作为名称模式。每个命中都通过 `getSource()` 读取实现并二次判断，必要时继续生成新的关键词。
...
NEO复用了MScan的核心入口识别思想。MScan面向微服务网关配置，其分析重点是网关路由与实际服务端点的对应关系。{% cite liu2025mscan %}

1. 定位 API Gateway 的配置文件；
2. 向LLM提供任务说明、配置到路由的示例和实际配置；
3. 让LLM输出真正会转发请求的路由正则列表，并理解 `SetResponseStatus=403`、`Denied` 等阻断过滤器；
4. 静态分析所有服务，提取端点 URI；
5. 用网关路由正则匹配端点 URI；
6. 只有匹配成功、实际暴露到网关的端点参数才成为外部用户 source。

该方法结合LLM网关配置解析、静态端点提取和路由匹配。XML可以作为配置载体，MScan论文的核心对象是Spring Cloud Gateway、Zuul、APISIX等网关路由。
...
此路径表示攻击者输入可能影响特权操作。路径是否真实可执行以及沿途授权是否充分，需要由第4步验证。
...
只找到 `authz()` 的名称无法证明授权充分。LLM还要读取实现，判断它验证登录状态或管理员角色等条件的方式，并评估该特权操作是否需要更细的所有权约束。论文关注已有检查与操作实际所需策略之间的语义差距。
...
4. **搜索原语模板。** 每种语言用独立CodeQL模块实现，Python只填充方法名、类型等占位符。统一的方法调用在Java、Python和JavaScript后端分别映射到 `MethodAccess`、`Call` 和 `CallExpr`。
...
作者只在PiggyMetrics上进行了人工审计：记录132次不同的原语执行，从 `Qname`、`Qast`、`Qflow`、`Qcg` 各抽5次，共检查20次。因此7/20不精确是小规模分层抽样结果，无法解释为全部查询存在35%错误率。

论文中的不精确主要指返回了正确关系的超集。这些结果包含有效关系，也包含额外候选。设真实运行时关系为 $R$，静态分析结果为 $A$，过度近似可写为：
...
作者只随机人工审计了69条LLM排除结果中的10条，这10条判断均正确。该结果表明抽样中未发现错误，无法证明69条结果全部正确；论文建议通过human-in-the-loop审查LLM给出的理由。
...
CodeQL* 对每个服务独立运行默认critical安全查询，没有跨服务权限提升模型，因此只检出7个，且都被NEO覆盖。通用查询套件没有定义应用特有特权操作、跨服务通道与授权充分性组成的检测问题，该实验主要反映查询任务和建模范围的差异。
...
该结果表明结构化检索能够降低Agent阅读代码的成本。平均18美元的单应用成本仍取决于模型价格、增量分析能力和CI运行频率。当前NEO每次面向整个应用构建和分析，尚未展示持续集成中的增量成本。

### 扩展到普通应用与其他漏洞类型

移除微服务假设、修改提示中的程序背景后，NEO 在普通 Python/Java 应用中发现 11 个新权限提升漏洞，其中 5 个获确认、2 个修复。

将prompt中的source、sink、sanitizer改为注入漏洞定义后，系统在JavaScript应用和包中发现5个命令注入和2个SQL注入，其中2个获开发者确认。Agent规划、结构化代码搜索和路径验证构成可复用骨架；不同漏洞类型仍需重新定义语义目标和净化条件。
...
该案例体现了LLM的语义推理作用。CodeQL能够证明 `orderNo` 流向订单更新，相邻方法中的所有权检查模式为付款操作需要检查订单所有者提供了强证据。

### 负责任披露

论文将§6.2与§6.6合计的42个新漏洞报告给维护者；投稿时18个获确认，8个已修复。24个零日权限提升漏洞对应主微服务评估，与扩展性实验中的新增发现分开统计。
...
传统SAST固化稳定知识，NEO对不稳定且应用特有的部分执行按需推理。四个搜索原语构成二者之间的接口，并以部分查询自由度换取Agent调用的稳定性、跨语言一致性与上下文效率。
...
各层功能互补。CodeQL的may-flow表示潜在数据流，LLM输出语义判断，Z3的 `sat` 表示抽象约束可满足。论文将这些能力组织为可迭代和可按需取证的Agent工作流。
